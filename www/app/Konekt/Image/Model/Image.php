...
class Konekt_Image_Model_Image
{
   ...
   {
      $imgdir = Konekt::app()->getConfigValue('konekt/image/directory');
   ...
   public function getResizeDimensions($entity, $group = NULL)
   {
      ...
      if (!isset($result[0]) || empty($result[0]))
      {
         ...
         $result['Mainheightfull']      = (int) Konekt::app()->getConfigValue('konekt/image/defaultHeight');
         $result['Mainwidththumbnail']  = (int) Konekt::app()->getConfigValue('konekt/image/defaultThumbnailWidth');
         $result['Mainheightthumbnail'] = (int) Konekt::app()->getConfigValue('konekt/image/defaultThumbnailHeight');
   ...
   {
      return "http://{$_SERVER['SERVER_NAME']}/". Konekt::LIB_ROOT_DIR. "/TimThumb/timthumb.php";
   }
   ...
   protected function _setMainImage($entity)
   {
      ...
      if (!$entityClass)
      {
         error_log("Konekt/Image: $entity is not a valid Entity");
         return false;
      }
      ...
      {
         error_log("Konekt/Image: $entityClass does not have an Image_id field");
         return false;
      }
   ...
   public function createAndAttachToEntity($tmpFile, $originalFileName, $entity, $group = NULL, $isMainImage = false)
   {
      ...
      if (!$entityClass)
      {
         throw new Exception("Konekt/Image: Could not create image, $entity is not a valid Entity");
         return null;
      }
